 **Структура** представляет собой _тип значения_, который может инкапсулировать данные и связанные функции. Для определения типа структуры используется ключевое слово `struct`:
 ```cs
struct Point
{
    public int X;
